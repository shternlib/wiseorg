# Intenture 1.4 - Master Release Document
...
Intenture Syntax строится как **двуслойный синтаксис**:
...
### 7.6. Structured Constraint Format

Типы Constraints [budget], [timeline] и [resource] поддерживают опциональный структурированный формат. Текстовое описание остается обязательным. Структурированные поля добавляются ниже описания с отступом.
...
1. Структурированные поля опциональны. Текстовый формат `- [budget] общий бюджет $5000` остается валидным.
2. Типы [safety], [legal], [quality], [coordination], [scope] остаются чисто текстовыми - им структурированный формат не нужен.
3. PM может предложить человеку формализовать бюджетные и временные ограничения, но не обязан требовать этого.
4. AI при наличии структурированных полей может автоматически трекать расход бюджета и соблюдение сроков.

### 7.7. Каноническая архитектура AI-facing form
...
### 7.9. Канонический шаблон AI-facing form
...
Lifecycle Model описывает жизненный цикл **intenture** как смыслового объекта.
...
| Structured -> Realizable | Пройден Critical Readiness Threshold | Все четыре условия CRT выполнены + Assumption Gate пройден (ни одного блока "Assumed by AI") + Supporting Blocks заполнены на уровне, достаточном для начала работы + Specialized Readiness Blocks для данного типа intenture выполнены |
...
> **Примечание:** Ссылки на внешние governance frameworks (VSM, ITIL и т.д.) вынесены за пределы core language. Intenture не зависит от конкретной организационной модели. Сигналы могут поступать из любого источника, релевантного контексту реализации.
...
> **Companion artifact:** Все три диаграммы оформлены в самодостаточном HTML-файле **Intenture_Visual_Notation.html**, который является частью release package языка.
...
### 11.4. Три пути разрешения Assumed by AI
...
Для блоков с Constraints типа [safety] и [legal] путь "Принятие как допущение" недопустим. Допущения в области safety и legal могут быть только подтверждены (-> Answered) или отклонены (-> Unknown Yet). PM обязан явно предупредить человека, если допущение касается safety или legal Constraint.
...
### 13.3. Assumption Gate

Intenture не может перейти в Realizable, если в Canvas существует хотя бы один блок со статусом "Assumed by AI". Это правило действует как обязательный mandatory gate между Universal CRT и Realization Decision.
...
Если хотя бы одно допущение отклонено и касается Critical Block или Specialized Readiness Block, переход в Realizable блокируется до закрытия Unknown Yet.
...
### 13.5. Specialized Readiness Matrix
...
|---|---|---|---|---|
| **Health Care Team** | Primary health goal; Baseline health data; Jurisdiction / legal constraints; Escalation rules к врачу; Expert role boundaries | Отсутствие primary health goal; Отсутствие baseline data; Неизвестная юрисдикция | Частота лабораторных тестов; Детали страхового покрытия | Universal CRT + Assumption Gate + все blocking fields Answered + expert roles согласованы |
| **AI Agent** | Allowed actions; Prohibited actions; Escalation rules; Failure conditions; Decision boundaries | Отсутствие allowed actions; Отсутствие escalation rules | Точные thresholds для edge cases; Детали мониторинга | Universal CRT + Assumption Gate + allowed/prohibited actions определены + escalation rules зафиксированы |
| **Product / Service** | Target user; Value proposition; First scope boundary; Success criteria / learning criteria; Key assumptions | Неопределенный target user; Отсутствие value proposition | Ценовая модель; Точный delivery timeline | Universal CRT + Assumption Gate + target user определен + value proposition сформулировано + first scope зафиксирован |

### 13.6. Принцип расширяемости
...
| 7 | **Confirm Assumptions** | Проводит все допущения AI через явное подтверждение человека | Все блоки со статусом "Assumed by AI" | Список допущений + запрос решения по каждому |
| 8 | **Decide Readiness** | Оценивает readiness по Universal CRT + Assumption Gate + Specialized Matrix, выносит решение | Readiness Layer в AI-facing form | Показ человеку Human-facing form + диаграммы + readiness decision |
| 9 | **Prepare Realization Form** | Формирует финальный AI-facing form, Object Card, Explication Record | Все блоки -> AI-facing form, Object Card, Explication Record | Показ человеку полного Human-facing представления для подтверждения |

### 14.2. Спецификация шага 7 - Confirm Assumptions
...
5. Для допущений, связанных с [safety] или [legal] Constraints, вариант "Принять как допущение" недоступен. PM явно объясняет, почему.
...
### 14.3. Принцип взаимодействия с человеком
...
- **Шаг 7** - список допущений AI + запрос решения по каждому;
- **Шаг 8** - Human-facing form + диаграммы + решение о readiness;
- **Шаг 9** - финальное представление для подтверждения перед стартом реализации.

Шаги 2 и 3 - внутренние операции PM, их результаты человек видит опосредованно через вопросы и представления.

### 14.4. Conflict Resolution Protocol
...
Четыре режима работы PM определяют, как PM ведет себя на разных стадиях жизни intenture.
...
- Assumption Gate пройден - ни одного блока "Assumed by AI" в Canvas;
...
**Protocol:** стандартный (Capture -> Extract -> Normalize -> Clarify -> Validate -> Stress-Test -> Confirm Assumptions -> Decide Readiness -> Prepare Realization Form).
...
- остальные шаги - стандартные (включая Confirm Assumptions).
...
- Assumed by AI блоки, заполненные из документа, требуют явного подтверждения через Assumption Gate.
...
5. Далее стандартные шаги: Normalize -> Clarify -> Validate -> Stress-Test -> Confirm Assumptions -> Decide Readiness.
...
Pattern Library - набор эталонных паттернов intenture для разных классов замыслов.

### 17.2. Unified Pattern Structure
...
### 18.4. Companion files

Все три примера доступны как отдельные companion files:

- **Example_1_Health_Care_Team.md** - Ready for Realization
- **Example_2_AI_Agent.md** - Ready for Structuring
- **Example_3_Product_Service.md** - Ready for Structuring

> **Примечание:** Примеры пока используют синтаксис v1.3. Обновление примеров до синтаксиса v1.4 (Accepted Assumption, Structured Constraints) запланировано как отдельная задача.

---

## 19. Practical Usage Guide
...
5. PM проводит Assumption Gate: показывает человеку все допущения AI, получает решение по каждому.
6. PM выносит readiness decision (Validate -> Stress-Test -> Confirm Assumptions -> Decide Readiness).
7. PM показывает человеку результат для подтверждения.
8. При достаточной определенности и подтверждении человека - начинается реализация.
9. При эволюции замысла - PM повторяет протокол, фиксируя новый Explication Record.
...
5. покажет вам все допущения, которые он сделал, и попросит подтвердить или отклонить каждое (Assumption Gate);
6. покажет вам Human-facing представление замысла;
7. вынесет readiness decision;
8. при вашем подтверждении - начнет реализацию.
...
- все допущения AI подтверждены или явно приняты человеком.

---

## 21. Scope Boundary

### Входит в Intenture 1.4 Core Language
1. Intenture Definition
2. Manifesto
3. Terminology Policy (с Language Policy для AI-facing form)
4. Glossary (с Assumption Gate, Accepted Assumption)
5. Meta-Model
6. Intenture Syntax (Intent Vocabulary, Constraint Types с [resource], Structured Constraint Format, Development Layer Syntax, Evidence/Metrics формат)
7. Lifecycle Model
8. Explication Record Versioning Policy
9. Visual Notation
10. Intenture Canvas (с шестью статусами, включая Accepted Assumption)
11. Intenture Object Card

### Входит в Intenture Environment 1.4
1. Readiness Checklist for Realization (с Assumption Gate, Specialized Readiness Matrix)
2. AI Interpretation Protocol (с Confirm Assumptions, Conflict Resolution Protocol)
3. PM Operating Modes
4. Entry Patterns (Greenfield, From Concept, Takeover + Reverse Explication)
5. Pattern Library
6. End-to-End Examples (3 companion files, синтаксис v1.3 - обновление запланировано)
...
| `Intenture_Vulnerabilities_Roadmap.md` | Известные уязвимости и план версий 1.5+ |
| `Intenture_Visual_Notation.html` | Три эталонных диаграммы Visual Notation |
| `Example_1_Health_Care_Team.md` | End-to-End Example: Health Care Team (Ready for Realization) |
| `Example_2_AI_Agent.md` | End-to-End Example: AI Agent (Ready for Structuring) |
| `Example_3_Product_Service.md` | End-to-End Example: Product / Service (Ready for Structuring) |
...
| 2 | Done | Glossary | Assumption Gate, Accepted Assumption добавлены. Reverse Explication. Conception vs Vision формализованы. |
| 3 | Done | Meta-Model | Explication Record как Core Entity. |
| 4 | Done | Intenture Syntax | Intent Vocabulary, Constraint Types (+ [resource]), Structured Constraint Format, Development Layer Syntax, Evidence/Metrics формат. |
| 5 | Done | Lifecycle Model | Lifecycle Transitions Table с триггерами и условиями. Assumption Gate добавлен в переход Structured -> Realizable. |
| 6 | Done | Explication Record Versioning Policy | Полная политика версионирования. |
| 7 | Done | Visual Notation | Примитивы, три эталонных диаграммы, пять типов диаграмм. |
| 8 | Done | Intenture Canvas | Single Source of Truth. Шесть статусов (включая Accepted Assumption). Три пути разрешения Assumed by AI. |
| 9 | Done | Intenture Object Card | Краткая карточка замысла. |
| 10 | Done | Readiness Checklist | Universal + Assumption Gate + Specialized readiness. Трехуровневая модель. |
| 11 | Done | AI Interpretation Protocol | Интегрирован с Canvas. 9-шаговый конвейер (шаг 7 - Confirm Assumptions). Conflict Resolution Protocol. |
| 12 | Done | PM Operating Modes | Четыре режима с entry/exit criteria. Assumption Gate в Structuring Mode. |
| 13 | Done | Entry Patterns | Greenfield, From Concept, Takeover + Reverse Explication. Все обновлены под Confirm Assumptions. |
| 14 | Done | Specialized Readiness Matrix | Три паттерна с дополнительными readiness blocks. Assumption Gate добавлен в условия. |
| 15 | Done | Pattern Library | Три эталонных паттерна. |
| 16 | Pending | End-to-End Examples | Companion files пока используют синтаксис v1.3. Обновление запланировано. |
| 17 | Done | Practical Usage Guide | Workflow с Assumption Gate. |
| 18 | Done | Quick Start Guide | Быстрый вход в язык с упоминанием Assumption Gate. |
| 19 | Done | Intenture Independence Principle | Язык не зависит от external frameworks. |
| 20 | Done | Release Notes | Вынесены в отдельный файл, обновлены для v1.4. |

---

## 23. Release Verdict

### Intenture 1.4 Core Language
**Ready**

### Intenture Environment 1.4
**Ready**
...
### Known Pending Items
- Обновление End-to-End Examples до синтаксиса v1.4

---

## 24. Final Release Statement

**Intenture 1.4** собран как:
...
- язык с operational environment (Protocol, Conflict Resolution, Assumption Gate, Modes, Entry Patterns, Readiness, Patterns, Examples);
- язык с Assumption Gate - обязательным safety-шлюзом, запрещающим переход к реализации при наличии неподтвержденных допущений AI;
- язык с формализованными ресурсными ограничениями (Structured Constraint Format для [budget], [timeline], [resource]);
